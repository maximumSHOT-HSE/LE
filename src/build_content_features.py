...
def get_parser():
    ...
    parser.add_argument("--save-path", type=str, required=True, help="Path to the csv file where built content features will be saved")
    return parser
...
def build_content_full_text(x):
    return (
        x["title"] + "[SEP]" + x["language"] + "[SEP]" + x["kind"] + "[SEP]" + x["description"] + "[SEP]" + x["text"]
    ).replace("\n", "*").replace("\t", "*")
...
def main(args):
    ...
    content_df.to_csv(args.save_path)
